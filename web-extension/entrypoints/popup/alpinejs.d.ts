declare module "@alpinejs/csp" {
  interface Alpine {
    ...
    interceptClone(
      callback: (
        from: Alpine.ElementWithXAttributes,
        to: Alpine.ElementWithXAttributes
      ) => void
    ): void;
    /**
    ...
    findClosest(
      el: Element,
      callback: (el: Alpine.ElementWithXAttributes) => boolean
    ): Element;
    /**
     ...
     */
    entangle<T_6>(
      outer: Alpine.GetterSetter<T_6>,
      inner: Alpine.GetterSetter<T_6>
    ): () => void;
    /**
    ...
    initTree(
      ...
      walker?: (
        el: Alpine.ElementWithXAttributes,
        callback: Alpine.WalkerCallback
      ) => any,
     ...
     */
    store<T extends keyof Alpine.Stores>(
      name: T,
      value: Alpine.Stores[T]
    ): void;
    /**
     ...
     */
    clone(
      oldEl: Alpine.ElementWithXAttributes,
      newEl: Alpine.ElementWithXAttributes
    ): void;
    /**
     * Clones the Alpine context of an element to another element
     * @internal
  ...
  type ElementWithXAttributes<T extends Element = HTMLElement> =
    withXAttributes<T>;
  ...
  type Bindings<T> = {
    [key in keyof T]: key extends `${
      | "x-on:"
      | "@"}${infer K extends keyof HTMLElementEventMap}`
  ...
  > = T extends InterceptorObject<infer U>
    ...
    ? T
    : {
        [K in keyof T]: InferInterceptor<
          T[K],
          D extends 1 ? 2 : D extends 2 ? 3 : D
        >;
      };
